...
% Maintained by jrn, SINTEF Applied Mathematics.
% $Date: 2009-10-01 15:05:27 +0200 (to, 01 okt 2009) $
% $Revision: 2920 $
...
%
%  a) Compute cornerpoint coordinates of each grid block
%    (buildCornerPtNodes)
%
...
%     cell pair, are processed separately (findfaults) from matching
%     pillar pairs (findFaces).
%
...
%     outer boundary.
%
%     Pinched faces are removed.
%
%  e) Compute connectivity in the k-direction (findVerticalFaces).  The
...
opt = struct('Verbose', false, 'Tolerance', 0.0, 'CheckGrid', true);
...
% TODO
% Assert that NaN junk occurs only in inactive cells
...
G.faceNodes        = uint32([]);
G.faces.numNodes   = uint8([]);
G.faces.neighbors  = zeros(0,2, 'uint32');
G.faces.tag        = uint8([]);
G.faces.cellTags   = [];
...
dispif(opt.Verbose, 'removing %d inactive and pinched cells\n', ...
       sum(~actnum(:)|G.cells.numFaces==0) - numAuxillaryCells);


% Remove auxillary and inactive cells
G = removeCells(G, find(~actnum(:)|G.cells.numFaces==0));
...
if opt.CheckGrid,
   assert(all(G.cells.numFaces>3));
   assert(all(G.faces.numNodes>2));
...
if numel(c) > 2,
   ...
   for i = 1:numel(c) - 1,
      g(i)  = removeCells(G, a(c(i):c(i+1)-1));               %#ok
...
for i = 1 : numel(c)-1,
   G(i).faces.nodePos = pos(G(i).faces.numNodes);
   G(i).cells.facePos = pos(G(i).cells.numFaces);
end
...
G.faces.num         = nf;
G.nodes.num         = size(G.nodes.coords, 1);
G.cells.numFaces    = accumarray(G.faces.neighbors(:)+1, 1, [G.cells.num+1, 1]);
G.cells.numFaces    = uint8(G.cells.numFaces(2:end));
...
[vec, ind]  = sort(vec(hf));
G.cellFaces = uint32([mod(vec-1, nf) + 1, cellTags(hf(ind))]);
...
% G.cellFaces = sortrows([G.faces.neighbors(:,1),(1:n)',  ones(n,1),cellTags(:,1);...
%                         G.faces.neighbors(:,2),(1:n)', -ones(n,1),cellTags(:,2)]);
% ind                 = G.cellFaces(:,1)==0;
% G.cellFaces(ind,:)  = [];
% G.cellFaces         = int32(G.cellFaces(:,[2,4]));
...
G.faceNodes        = [G.faceNodes;        uint32(F)];                   %#ok
G.faces.neighbors  = [G.faces.neighbors;  uint32([c1(:), c2(:)])];      %#ok
G.faces.numNodes   = [G.faces.numNodes;   uint8(nF)];                   %#ok
G.faces.tag        = [G.faces.tag;        zeros(n, 1, 'uint8')];        %#ok
...
n          =  numel(c1);
dispif(opt.Verbose, ' Found %d new regular faces\n', n);
G.faceNodes        = [G.faceNodes;        uint32(reshape(f', [], 1))];
G.faces.neighbors  = [G.faces.neighbors;  uint32([c1(:), c2(:)])];
G.faces.numNodes   = [G.faces.numNodes;   repmat(uint8(4), [n, 1])];
G.faces.tag        = [G.faces.tag;        zeros(n, 1, 'uint8')];
G.faces.cellTags   = [G.faces.cellTags;   repmat(tags, [numel(c1), 1])];
...
G.faces.neighbors  = [G.faces.neighbors;  uint32(new)];                 %#ok
...
G.faces.numNodes = [G.faces.numNodes; uint8(ncor)];                  %#ok
G.faceNodes      = [G.faceNodes;      uint32(cor) ];                 %#ok
G.faces.tag      = [G.faces.tag;      ones(numel(ncor), 1, 'uint8')];%#ok
...
%
% Given: 
% vectors of point numbers a and b where a(:,1), b(:,2) refer to pillar (1)
