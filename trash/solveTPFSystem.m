function resSol = solveTPFSystem(resSol, G, S, rock, fluid, varargin)
% solveTPFSystem -- Solve mixed pressure linear system
%                   for models without wells.
%
% SYNOPSIS:
%   resSol = solveTPFSystem(resSol, G, S, rock, fluid)
%   resSol = solveTPFSystem(resSol, G, S, rock, fluid, 'pn', pv, ...)
%
% PARAMETERS:
...
%   S       - System structure.
%
%   rock    - Structure with valid field 'perm'.
%
...
%
% RETURNS:
%   resSol - Solution structure having the following fields:
%              - cellPressure  : Pressure corresponding to cells.
%              - facePressure  : Pressure on external Neumann faces.
%              - cellFlux      : Flux corresponding to cell-faces as in
%                                G.cellFaces.
%              - faceFlux      : Flux corresponding to faces.
%              - A             : System matrix.
%                                Only returned if 'MatrixOutput' is set.
%
% SEE ALSO:
%   assembleMimeticSystem, initResSol, initSimpleFluid, solveMixedSystem.
%
% NOTE:
...
%   a model with no wells.  Use function 'solveWellSystem' to solve models
%   containing wells, though 'solveWellSystem' does not support TPFA
%   discretizations.

% $Id: solveTPFSystem.m 1773 2009-03-19 12:47:11Z bska $
...
if ~strcmp(S.type, 'mixed'),
   error(id('Type:NotMixed'), ...
         'System has to be of type ''mixed'' to use solveTPFSystem')
end
...
[res{1 : nres}] = tpfSymm(B, C, D(:,nF), f, g, h(nF), Do, ...
                          'Regularize', regul);
tocif(verbose)
