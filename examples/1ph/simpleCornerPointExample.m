...
% domain.
rock.perm = logNormLayers(nc, [100, 400, 50]) .* milli*darcy();
...
rSol = solveIncompFlow(rSol, initWellSol([],0.0), G, S, ...
   fluid, 'MatrixOutput', true, 'src', [], 'bc', bc);
...
subplot('Position',[0.01 0.25 0.48 0.5]),
   plotCellData(G, rSol.cellPressure ./ barsa());
   title('Pressure [bar]')
   view(3), camproj perspective, axis tight off, camlight headlight

subplot('Position',[0.51 0.25 0.48 0.5]),
   cellNo  = rldecode(1:G.cells.num, double(G.cells.numFaces), 2) .';
   plotCellData(G, sqrt(accumarray(cellNo, abs(convertTo(rSol.cellFlux, meter^3/day)))));
...
% <font size="-1">
%   Last time modified:
%   $Id: simpleCornerPointExample.m 1945 2009-03-31 09:22:39Z bska $
% </font>
% </html>
