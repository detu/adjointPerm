...
rock.perm = repmat(100, [G.cells.num, 1]) .* milli*darcy;
...
W = addWell(G, rock,  W, cellsWell2, 'Type', 'bhp', ...
...
wellSol = initWellSol(W, 1.0*barsa());
display(wellSol);
...
gravity off
[resSol, wellSol] = solveIncompFlow(resSol, wellSol, ...
                                    G, S, fluid, 'wells', W);
...
subplot(2,2,2)
   plot(convertTo(-wellSol(2).flux, meter^3/day))
   title('Producer inflow profile [m^3/d]');

subplot(2,2,3)
   plotCellData(G, convertTo(resSol.cellPressure, barsa));
   title('Pressure [bar]')
   view(3), camproj perspective, axis tight off, camlight headlight

subplot(2,2,4)
   cellNo = rldecode(1:G.cells.num, double(G.cells.numFaces), 2) .';
   cf     = accumarray(cellNo, abs(resSol.cellFlux) );
...
% <font size="-1">
%   Last time modified: 
%   $Id: simpleWellExample.m 2072 2009-04-21 17:24:42Z bska $
% </font>
% </html>
