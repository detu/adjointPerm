...
rock.perm      = rock.perm .* milli*darcy;  % mD -> m^2
...
xrRef = solveIncompFlow(initResSol(G, 0),            ...
                        initWellSol([], 0.0), G, S,  ...
                        fluid, 'MatrixOutput', true, ...
                        'bc', bc);

CS = generateCoarseSystem(G, rock, S, CG, ...
                          fluid.mob(struct('s',ones([G.cells.num, 1]))), ...
                          'bc', bc, 'Verbose', verbose);
xrMs =  solveIncompFlowMS(initResSol(G, 0),           ...
                          initWellSol([], 0.0),       ...
                          G, CG, p, S, CS, fluid,     ...
                          'bc', bc, 'Solver', S.type, ...
                          'MatrixOutput', true);
...
subplot('Position',[0.02 0.52 0.46 0.42]),
   plot_pres(xrRef); shading faceted, title('Pressure Fine [bar]')
   view(3), camproj perspective, axis tight off, camlight headlight
   cax = caxis; colorbar
...
subplot('Position',[0.02 0.02 0.46 0.42]),
   plot_flux(xrRef); shading faceted, title('Flux intensity Fine')
   view(3), camproj perspective, axis tight off, camlight headlight
   cax2 = caxis; colorbar
...
bf = S.BI * extractBF({ CS.basis{faces} }, size(G.cellFaces,1), CG);
...
d = sparse(cellNo, 1 : size(G.cellFaces,1), 1) * abs(bf);
...
% <font size="-1">
%   Last time modified: 
%   $Id: simpleCornerPointExampleMS.m 1945 2009-03-31 09:22:39Z bska $
% </font>
% </html>
