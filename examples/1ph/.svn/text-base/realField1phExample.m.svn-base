...
rock.perm  = K(G.cells.indexMap)*milli*darcy; clear K;
...
for i = 1 : numel(I),
   W = verticalWell(W, G, rock, I(i), J(i), 1:nz, 'Type', 'rate', ...
                    'Val', R(i), 'Radius', radius, 'Comp_i', [1,0,0], ...
                    'name', ['I$_{', int2str(i), '}$']);
end
...
clf
   ...
   plotCellData(G, sqrt(accumarray(cellNo,  ...
      abs(convertTo(rSol.cellFlux, meter^3/day)))),'EdgeColor', 'k');
   plotWell(G,W,'height',200,'color','c');
   colorbar('horiz'), axis tight off; view(20,80)
   zoom(2)
...
clf
   ...
   plotWell(G,W,'height',200,'color','c');
   colorbar('horiz'), axis tight off; view(20,70)
   zoom(2)
...
% <font size="-1">
%   Last time modified:
%   $Id: realField1phExample.m 2072 2009-04-21 17:24:42Z bska $
% </font>
% </html>
