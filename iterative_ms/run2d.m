...
if ~heterogen,
   ...
   poro = repmat(expm(smoother) * rand([nc, 1]), 1, cellDims(2));
   [X,Y]=meshgrid(linspace(-1,1,cellDims(1)), linspace(-1,1,cellDims(2)));
   
   poro = 0.3+0.05*sin(4*X*pi) - 0.05*cos(5*Y*pi);
   %perm = exp(40 * poro - 10).*milli*darcy;
   perm = exp(40 * poro - 5).*milli*darcy;
...
pLo     = 90.0 * barsa();
initP   = pLo;
bc      = pside(bc, G, 'LEFT',  1:G.cartDims(2), 1:G.cartDims(3), pHi, 'sat', 1);
bc      = pside(bc, G, 'RIGHT', 1:G.cartDims(2), 1:G.cartDims(3), pLo, 'sat', 1);
...
CS  = generateCoarseSystem(G, rock, S, CG, mob, 'bc', bc, 'src', src, 'BasisWeighting', 'poros', 'Overlap', 0);
...
for step = 1 : numSteps
   ...
   subplot(3,2,[1 3]),
      ...
      plot(X, convertTo([xrRef.cellPressure, xrMs.cellPressure], barsa), 'o')
   ...
   cla, hold on
      plot(X, (massRef-massMs)./massRef, 'o');
   ...
   subplot(3,2,2),
      ...
      %plot(X,ff(1 : Nc, :)*[1 -1]'/norm(ff(1 : Nc, 1)), 'o')
      plot(X,ff(1 : Nc, :), 'o')
      title('Total flux'), 
      %set(gca, 'XTick', []),
      set(gca, 'FontSize', FontSz);
