function resSol = thetaTransport(resSol, wellSol, G, tf, ...
                                    rock, fluid, varargin)
...
% $Id: implicitTransport.m 2044 2009-04-20 17:18:53Z bska $
...
[g, flux, q, pv, dt] = initTransport(G, resSol, wellSol,              ...
                                 rock, fluid, 'ComputeDt', (opt.theta==0), ...
...
if norm(g_vec(1 : size(G.nodes.coords,2))) > 0,   
   error('thetaTransport:gravity:Unsupported', ...
            'Gravity is not supported at the moment');
         %resSol = twophaseUpwBEGrav(resSol, G, tf, q, flux, g, ...
   %                           pv, fluid, arg{:});
else   
   opt.dt = dt;
   
   if opt.theta
      resSol = twophaseUpwTheta(resSol, tf, q, flux, pv, fluid, arg{:});
   else      
      satWarn  = sqrt(eps);      
