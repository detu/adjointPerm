function resSol = twophaseUpwBE(resSol, tf, q, gm, pv, fluid, varargin)
%Implicit single point upwind solver for two-phase flow, no gravity.
%
% SYNOPSIS:
%   resSol = twophaseUpwBE(resSol, t, q, gm, porvol, fluid)
%   resSol = twophaseUpwBE(resSol, t, q, gm, porvol, fluid, 'pn1', pv1, ...)
%
% DESCRIPTION:
...
% $Date: 2009-09-09 10:23:54 +0200 (on, 09 sep 2009) $
% $Revision: 2695 $
...
F   = @(sol, s0, dt) (sol.s(:,1) - s0 + dt.* ...
                     (v*f(fluid.kr(sol)) - max(q./pv,0)));
Jac = @(sol, dt) ( speye(nc) + dt.*v*sparse(1:nc, 1:nc, ...
