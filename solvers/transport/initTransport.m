function [gravmat, flux, q, pv, dt] = initTransport(G, resSol, wellSol, ...
                                                    rock, fluid, varargin)
...
% SYNOPSIS:
%   [gravmat, ...
%    flux, q, pv, dt] = initTransport(G, resSol, wellSol, rock, fluid)
%   [gravmat, ...
%    flux, q, pv, dt] = initTransport(G, resSol, wellSol,
%                                     rock, fluid, 'pn1', pv1, ...)
%
% PARAMETERS:
%   G       - Grid data structure discretising the reservoir model.
%
%   resSol  - Reservoir solution structure.  Pressures are assumed to be
...
%   twophaseUpwFE, twophaseUpwFEGrav, twophaseUpwBE, twophaseUpwBEGrav.

%{
#COPYRIGHT#
%}

% $Id: initTransport.m 2338 2009-06-05 17:19:30Z bska $
   ...
   if norm(g_vec(1 : size(G.nodes.coords,2))) > 0,
      [gravmat, flux] = matrices(G, rock, fluid, resSol, ...
                                 opt.OnlyGrav, is_int);
   else
      [gravmat, flux] = matrices_nograv(G, resSol, is_int);
   end
   ...
   if ~isempty(opt.wells),
      [i, s] = contrib_wells(opt.wells, wellSol);
      qi = [qi; i];
      qs = [qs; s];
   end

   if ~isempty(opt.src), assert (~isempty(opt.src.sat))
      [i, s] = contrib_src(opt.src);
      qi = [qi; i];
      qs = [qs; s];
   end

   if ~isempty(opt.bc), assert (~isempty(opt.bc.sat))
      [i, s] = contrib_bc(G, resSol, opt.bc, is_int);
      qi = [qi; i];
      qs = [qs; s];
   end
   ...
   if opt.ComputeDt,
      ...
      deriv = @(f) diff(f(fluid.mu, fluid.kr(struct('s', s)))) ./ diff(s);
...
function [gravity_matrix, flux] = matrices_nograv(G, resSol, is_int)
...
%                    of gravity effects.
   ...
   % - Flux matrix -
   % resSol.cellFlux is amount of outflow for all half contacts on all
...
function [gravity_matrix, flux] = matrices(G, rock, fluid, resSol, ...
                                           grav_only, is_int)
...
% is_int - Logical vector of length G.faces.num, true for internal face.
   ...
   if grav_only,
      ...
      flux = sparse(cellNo(cIntFInx), locfix, ...
                    resSol.cellFlux(cIntFInx), nc, nf);
   end
end
...
%--------------------------------------------------------------------------
%
% Source term contributions from wells, boundary conditions and explict
% sources.
%
%--------------------------------------------------------------------------
%
% We need special treatment for injection into reservoir.  Specifically, we
% need to take into account the fluid composition of the injected fluid
% lest the subsequent transport solve (using e.g. 'twophaseUpwFE') be
% thoroughly confused.
%
