% Lagrangian Barrier Method
% Analyzing of Observability 
% This analysis is similar to "output constraint" in the sense
...
objectiveFunction = str2func('PermMismatchLB');
...
param.lamda       = 1;
param.tau         = 0.1;
param.mu          = 1e-1;    % initial penalty parameter
...
omegaStar         = 1e-8;
tauStar           = 9.5e-5;   % the smallest constraint violation is preferable..
...
while gradP >= omegaStar
    ...
    % STEP 1
    % inner iteration
    options = optimset('Display', 'iter', 'Hessian', 'bfgs', ...
    ...
    % compute updated Aug-Lag multiplier
    param.lamda = param.mu * param.lamda / constraint;
