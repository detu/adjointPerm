...
d2fw     = fluid.d2fw(simRes(curStep).resSol);
dltInv   = fluid.dLtInv(simRes(curStep).resSol);
d2ltInv  = fluid.d2LtInv(simRes(curStep).resSol);
...
if (curStep < numSteps)
    ...
    RHS     = secAdjRes(curStep+1).resSol.s;
...
%---------- Solve system --------------------------------------------------
secAdjRes(curStep).resSol.s  =  systMat \ RHS;
