function [obj, valR] = PermMismatchPB(param, G, S, W, rock, fluid, simRes, schedule, controls, varargin)
...
for step = 2 : numSteps
    ...
    if step == numSteps
        ...
        % define waterCut
        barrierTerm = param.mu * log(fwBound - waterCut);
        val = val + barrierTerm;
    ...
    if computePartials        
        ...
        if step == numSteps
            ...
            ds( wellCells(prodInx) )   = param.mu * log(exp(1)) * deriv / (fwBound - waterCut);
            partials(step).s           = ds;
        else
