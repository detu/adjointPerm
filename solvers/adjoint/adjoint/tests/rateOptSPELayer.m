...
W = addWell(G, rock, W, 1          , 'Type', 'rate', 'Val',  -.25/day , 'Radius', radius, 'Name', 'p1');
W = addWell(G, rock, W, nx         , 'Type', 'rate', 'Val',  -.25/day , 'Radius', radius, 'Name', 'p2');
W = addWell(G, rock, W, nx*ny-nx+1 , 'Type', 'rate', 'Val',  -.25/day , 'Radius', radius, 'Name', 'p3');
W = addWell(G, rock, W, nx*ny      , 'Type', 'rate', 'Val',  -.25/day , 'Radius', radius, 'Name', 'p4');

W = assembleWellSystem(G, W, 'Type', 'comp_hybrid');

% Schedule: one time step, [0 1 PVI]
totVol   = sum(G.cells.volumes.*rock.poro);
schedule = initSchedule(W, 'NumSteps', 10, 'TotalTime', .4*totVol*day/10, ...
                        'Verbose', verbose);
