function [simRes, schedule, controls, data] = lineSearch(...
...
maxStepSizes          = stepSize*ones( size(uInit) );
...
gri         = 2/(1 + sqrt(5));  % inverse golden ratio
...
for k = 2 : numPnts
    ...
    if or(and(k==3, pos == 4), and(k > 5, x(1)~=0) ), break; end
end
...
for cStep = 1 : nCSteps
    grad{cStep} = ( (fac.^2)/max(abs(obj.val),1) ) .* grad{cStep};
end
