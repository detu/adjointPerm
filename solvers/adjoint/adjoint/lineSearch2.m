function [simRes, schedule, controls, data] = lineSearch2(...
...
%fixedControls = false(size(fixedControls));                
% Find projected gradient, i.e. feasable direction..
%newFixedControls = fixedControls;
%while any( newFixedControls )
    
    FC = reshape(fixedControls, numControlWells, numSteps);
    MN = reshape( minMax(:, 1) - uInit, numControlWells, numSteps );
    MX = reshape( minMax(:, 2) - uInit, numControlWells, numSteps );
...
for step = 1 : numSteps
        ...
        mx              = MX(:, step);
        g               = grad{step};
        gradStep{step}  = projectGrad(controls, stepSize*g, fc, mn, mx);
%         
%  
%         if any(fc)
%             g          = grad{step};
%             grad{step} = projection(controls, fc, minVals, maxVals) * g;
%         end
%     end
%     %Recompute
%     du      = cell2mat( {grad{:}}');
...
% Update data-struct
data.value       = val;
data.relNormGrad = norm(duStep, 'inf')/norm(uCur, 'inf');
data.success     = success;
data.fraction    = alph;
data.stepSize    = alph*stepSize;
...
function  pGrad = projectGrad(controls, grad, fc, mn, mx)
disp('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX')
projector = projection(controls, fc);
res = Inf;
pGrad = grad;
ng  = norm(pGrad);
while res*ng > 1e-12
    pGradPrev = pGrad;
    pGrad = min( max(pGrad, mn), mx)
    pGrad = projector * pGrad
    res   = norm( pGradPrev - pGrad );
end
