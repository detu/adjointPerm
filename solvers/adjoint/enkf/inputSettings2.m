%----------------------------------------------------------------------------------
% DESCRIPTION:
%   Example input file for mrstEnKF. This example: 2D reservoir section connecting
%   1 injection well and 1 producer well. The position, slope and throw of
%   a fault between the 2 wells is assumed to be uncertain.
%
% AUTHOR: 
...
%----------------------------------------------------------------------------------
 ...
 transform      = {1 0 ...          % transformation functions for the states
                   0 [2 0.2 0.8] 0};% (see applyTransform.m)
...
% Time steps
%---------------------------------------------------------------------------------- 
 tsim           =  15.00*year();    % simulation end time
 tend           =  10.00*year();    % history match end time
 dt             =   0.50*year();    % simulation time step
 dto            =   1.00*year();    % observation time interval (multiple of dt)
 dtu            =  10.00*year();    % update time interval
