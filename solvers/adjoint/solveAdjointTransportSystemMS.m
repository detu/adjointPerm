function [adjRes] = solveAdjointTransportSystemMS(G, SG, S, W, rock, fluid, simRes, adjRes, obj, varargin)
...
%At      = generateUpstreamTransportMatrixMS(G, S, W, SG, simRes(curStep).resSol, ...
%                              simRes(curStep).wellSol, 'Transpose', true);
...
if curStep < numSteps
    ...
    dim     = DLtInv(simRes(curStep).resSol);
...
% Solve system
systMat_c    = SG.cells.subCells' * systMat * SG.cells.subCells;
RHS_c        = SG.cells.subCells' * RHS;
adjRes(curStep).resSol.s  = SG.cells.subCells * (systMat_c \ RHS_c);
