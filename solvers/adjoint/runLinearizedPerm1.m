function [linSimRes] = runLinearizedPerm1(simRes, G, S, W, rock, fluid, schedule, deltaU, varargin)
...
%     S.C     = sparse(1:numel(cellNo), cellNo, 1);
    ...
    f_bc = f_bc - S.DK * spdiags( (S.C*LtInv).*v , 0, numCF, numCF)* S.C * deltaU;
    ...
    for wellNr = 1:numW
        ...
        W(wellNr).S.RHS.f =  W(wellNr).S.RHS.f + w.S.DK * spdiags(( ( (w.S.C*LtInv).*q ) )) * w.S.C * deltaU;  % permeability adjoint
    end
