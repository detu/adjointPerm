...
% $Id: evalBasisFuncGlobal.m 2933 2009-10-01 15:38:47Z bska $
   ...
   opt = struct('Verbose', false, 'Overlap', 0, ...
                'src', [], 'bc', [], ...
                'LinSolve',   @mldivide);
   ...
   if opt.Verbose,
      h = waitbar(0, 'Computing flux and pressure basis functions using global info...');
      nFaces = numel(faces);
      fprintf('Computing flux and pressure basis functions using global info ... '), tic
   end
   ...
   for f = 1 : numel(faces),
      ...
      if nblk == 1, 
         ...
         % the basis for face.
         sH = handle_global(sH, g, global_inf, iH, sub_map.sub_f(face), i);   
        ...
        do_reg = ~any(is_dir);  % Need to set pressure zero level?
          [v, p, lam(~is_dir)] = schurComplementSymm(sBI, sC, sD(:,~is_dir), ...
                       sF , sG, sH(  ~is_dir), ...
                       'Regularize', do_reg, 'LinSolve', opt.LinSolve);            
   nnz(sH)
...
function [sF, sH, lam, is_dir] = handle_bc(sF, sH, lam, is_dir, ...
   ...
   loc_iF      = is_dir(fno(g.cellFaces(iF,1)));
...
function [sH] = handle_global(sH, g, global_flux, iH, ih, i)
   ...
   % Neumann (flux) boundary conditions.
   sH(is_neu)  = global_flux(ih);
   denom = sum(sH(is_neu));
   if abs(denom) > sqrt(eps(denom)),
      sH(is_neu) = sgn*sH(is_neu) ./ denom;
   end
